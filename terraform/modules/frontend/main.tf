...
variable "api_gateway_url" {
  ...
  type        = string
  default     = "https://m8vbgbh2hl.execute-api.us-east-1.amazonaws.com"
}
...
function handler(event) {
    ...
    headers['content-security-policy'] = { 
        value: "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self' ${var.api_gateway_url}; font-src 'self' data:;" 
    };
...
EOT
  ...
  filename = "${path.root}/../health-hub-frontend/.env.${var.environment}"
}
