...
export const convertTextToSpeech: APIGatewayProxyHandler = async (event) => {
  try {
    ...
    const audioUrl = await aiInteractionService.convertTextToSpeech(text);
    ...
    return {
      ...
      body: JSON.stringify({ error: "Could not convert text to speech" }),
    };
  }
};
